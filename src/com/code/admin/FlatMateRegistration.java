...
public class FlatMateRegistration extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try 
		{
			PreparedStatement ps = con.prepareStatement("select * from `flatmate_details` where email=?");
			...
			if (i>0) 
			{
				PreparedStatement ps2 = con.prepareStatement("UPDATE `payment_details` SET action='Added' WHERE email = '"+email+"'");
				ps2.executeUpdate();
				System.out.println("Registration Done");
				response.sendRedirect("OwnerHome.jsp?add");
			}
			}
			else 
			{
